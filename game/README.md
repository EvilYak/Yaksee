...
**Démo en ligne :** https://evilyak.github.io/Yaksee/ (branche `gh-pages`, à
republier après chaque changement — voir plus bas).
...
### Republier la démo GitHub Pages

```bash
cd game
npm run build
cd ..
git worktree add /tmp/yaksee-ghpages gh-pages   # si le worktree n'existe pas déjà
rm -rf /tmp/yaksee-ghpages/*
cp -r game/dist/* /tmp/yaksee-ghpages/
touch /tmp/yaksee-ghpages/.nojekyll
cd /tmp/yaksee-ghpages && git add -A && git commit -m "Republish" && git push
```
